...
class Spider:
    ...
    async def playerContent(self, flag, pid, vipFlags):
        url = await self.get_play_data(pid)
    ...
    async def get_data(self, url):
        data = []
        try:
            res = await entry.api_json(url, headers_dict=self.headers, _return_response=True)
            if res['status'] != 200:
                return data
            ...
            print(e)
        return data
    async def get_detail_data(self, ids):
        ...
        try:
            res = await entry.api_json(url, headers_dict=headers, _return_response=True)
            if res['status'] != 200:
                return []
            i = res['json']['data']
            urls = []
            ...
            print(e)
        return []
    async def get_play_data(self, play):
        info = play.split('-')
        _id = info[0]
        _pid = info[1]
