...
# 导入主程序提供的工具（CF Workers 中这些在全局可用）
try:
    from entry import api_json, fetch_raw, build_url, _timestamp_ms
except ImportError:
    # 这些函数在 entry.py 的全局作用域中
    pass
...
class JianpianSpider:
    ...
    async def categoryContent(self, tid, pg, filter_flag, extend):
        ...
        data = await entry.api_json(
            ...
            params=params, headers_dict=self._headers()
        )
        videos = []
        for item in data.get('data', []):
            videos.append({
    ...
    async def detailContent(self, ids):
        ...
        vod = {
            'vod_id': str(vid),
            'type_name': '/'.join([t.get('name','') for t in res.get('types',[])]),
    ...
    async def searchContent(self, key, quick="", pg="1"):
        import entry
        ihost = await self._get_ihost()
        params = {'key':key,'category_id':'88','page':str(pg),'pageSize':'20'}
        data = await entry.api_json(
